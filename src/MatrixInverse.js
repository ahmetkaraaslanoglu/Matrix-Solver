...
const MatrixInverse = ({navigation}) => {
    ...
    function matrixInverse (){
        let transpozeMatrix = transposeMatrix(cofactorMatrix(matrixArr));
        let detA = determinant(matrixArr);
        const InverseMatrix = [];
        for (let i = 0; i < transpozeMatrix.length; i++) {
            const row = [];
            for (let j = 0; j < transpozeMatrix.length; j++) {
                let element = ( 1 / detA ) * Number(transpozeMatrix[i][j]);
                if (element % 1 !== 0){
    ...
    return(
        <View style={{flex:1}}>
            ...
            <View style={{flex:1,backgroundColor:'black',justifyContent:'center',alignItems:'center',flexDirection:'row'}}>
                ...
                <Text style={{color:'white',fontSize:24}}>Matrix Inverse</Text>
            </View>
            ...
            <View style={{flex:1,backgroundColor:'white',justifyContent:'center',alignItems:'center'}}>
                <TouchableOpacity style={{height:50,width:300,backgroundColor:'black',justifyContent:'center',alignItems:'center',borderRadius:40}}
                                  onPress={() => {
                                      setSolutionMatrix(matrixInverse());
                                  }}
                >
                    <Text style={{fontSize:20,color:'white'}}>Matrisin Tersini Hesapla</Text>
                </TouchableOpacity>
